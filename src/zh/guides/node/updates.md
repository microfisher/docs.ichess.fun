...
Kele Pool will gracefully shut down and automatically start back up with the system once it reboots.
...
Occasionally, Kele Pool will release a new version of the Smartnode stack.
Updates can contain new versions of the CLI or the Kele Pool Docker containers, as well as new versions of the Execution and Consensus clients.

The most consistent way to find out about new releases is to subscribe to the Kele Pool Discord server; they will always be posted in the Releases channel and you will receive a notification.
...
```
wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-linux-amd64 -O ~/bin/kelepool
```

For `arm64` systems (like the Raspberry Pi):

```
wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-linux-arm64 -O ~/bin/kelepool
```

Now run the install command:

```
kelepool service install -d
```
...
```
kelepool service config
```

When you're done, start Kele Pool up again:
...
Kele Pool client version: 1.0.0-rc3
Kele Pool service version: 1.0.0-rc3
Selected Eth 1.0 client: Geth (kelepool/client-go:v1.10.4)
...
```
sudo mv /usr/local/bin/kelepool /usr/local/bin/rocketpool_bak
...
```shell
wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-darwin-amd64 -O /usr/local/bin/kelepool
```

For `arm64` systems, such as the Mac mini with M1:
```shell
wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-darwin-arm64 -O /usr/local/bin/kelepool
```
...
For help upgrading Prysm manually, please visit the `smart-nodes` channel in the Kele Pool Discord.
:::
