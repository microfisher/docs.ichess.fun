# Creating a Standard Kele Pool Node with Docker

In this section, we will walk through the process of installing the Kele Pool Smartnode stack using the standard [Docker](https://www.docker.com/resources/what-container)-based setup. 
This will install and configure everything you need to run a complete node, including:
- The Kele Pool **Smartnode** software
...
At a high level, here's what is involved in installing Kele Pool:

1. Download the Kele Pool command-line interface (CLI)
1. Use the CLI to install the Smartnode stack
1. Configure the Smartnode stack with an easy-to-use UI in the terminal
1. Done!


## Downloading the Kele Pool CLI

The instructions for downloading the CLI vary based on your Operating System.

::: warning NOTE
You must perform the following instructions **on the machine you will use for your Kele Pool node.**
...
```
```
kelepool version 1.3.0
```

::: tip TIP
If you see an error message like this,
```
-bash: /home/user/kelepool: cannot execute binary file: Exec format error
```
...
If your system is neither of those, then you will not be able to run Kele Pool.
:::::
...
```shell
curl -L https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-darwin-amd64 > /usr/local/bin/kelepool
```

For `arm64` systems, such as the Mac mini with M1:
```shell
curl -L https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-darwin-arm64 > /opt/homebrew/bin/kelepool
```

For `x64` systems (most Macs):
Mark it as executable, so it has permissions to run:
```shell
chmod +x /usr/local/bin/kelepool
```

For `arm64` systems, such as the Mac mini with M1:
Mark it as executable, so it has permissions to run:
```shell
chmod +x /opt/homebrew/bin/kelepool
```
...
If you see an error message like this,
```
/usr/local/bin/kelepool: Exec format error
```
...
Step 6 of 8: Creating Kele Pool user data directory...
Step 7 of 8: Downloading Kele Pool package files...
Step 8 of 8: Copying package files to Kele Pool user data directory...

The Kele Pool service was successfully installed!
```

If there aren't any error messages, then the installation was successful.
By default, it will be put into the `~/.kelepool` directory inside of your user account's home folder.
