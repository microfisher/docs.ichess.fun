# Starting Kele Pool and Setting Up a Node Wallet

At this point, you should have the complete Kele Pool infrastructure running, including the Smartnode stack, an Execution (ETH1) and an Consensus (ETH2) client.
You should also have hardened your operating system from outside attackers.
If you've completed both of these steps, you're ready to create a Kele Pool node and begin staking.
If not, please review the previous sections and return here once you've completed those steps.


## Starting and Stopping the Kele Pool Services
...
```
kelepool service start
```

This command will create all of the necessary Docker images if they don't already exist, make sure the Kele Pool docker network and storage volumes have been initialized, and update any containers if they no longer match the configuration settings from `kelepool service config`'s UI.
...
If you ever need to stop the services (for example, during an upgrade or because you need to do maintenance), you can use `kelepool service stop` to shut everything down.
The output should look like this:

```
$ kelepool service stop

Are you sure you want to pause the Kele Pool service? Any staking minipools will be penalized! [y/n]
y
...
::: tip NOTE
Once you call this, Kele Pool will not automatically start after a system reboot.
You will have to call `kelepool service start` to start all of the Docker containers and enable auto-start on reboot again.
...
Note that your external Execution (ETH1) and/or Consensus (ETH2) clients will not be listed here, as Kele Pool does not manage them and thus cannot start them.
...
:::
::: tab Native Mode
In native mode, you already set up and started the Smartnode services as part of the [Configuring a Native Kele Pool Node without Docker](./native.md) section.
...
Kele Pool client version: 1.0.0-pre
Kele Pool service version: 1.0.0-pre
...
If you are not on the network you expect to be on, go back to the Installing Kele Pool section and review the installation instructions - you may have missed the portion that has different instructions depending on which network you want to use.
...
The second set of lines will tell you which clients you're using, and which versions of them are defined in Kele Pool's `config.yml` file.
...
a3c22f54eff0   kelepool/smartnode:v1.0.0-rc3     "/go/bin/kelepool …"   2 hours ago   Up 2 minutes                                                                                                                 rocketpool_node
0d5818868ef6   statusim/nimbus-eth2:amd64-v1.3.0   "sh /setup/start-val…"   2 hours ago   Up 2 minutes                                                                                                                 rocketpool_validator
88bea525fa89   kelepool/smartnode:v1.0.0-rc3     "/go/bin/kelepool …"   2 hours ago   Up 2 minutes                                                                                                                 rocketpool_watchtower
...
62314e5a0ecf   kelepool/smartnode:v1.0.0-rc3     "/bin/sleep infinity"    2 hours ago   Up 2 minutes                                                                                                                 rocketpool_api
...
This is okay!
The validator container will fail until a new validator has been created using the `kelepool node deposit` command, which we will cover in the [Creating a Minipool (ETH2 Validator)](./create-validator.md) section.
...
To check on the logs, use the `kelepool service logs <container>` command, where `<container>` is one of `eth1`, `eth2`, `validator`, `api`, `node`, or `watchtower`.
...
You can ignore `eth1` and/or `eth2` based on which ones you are maintaining externally, but if you are letting Kele Pool manage one of them, you should check the status of it now using this command.
::::
...
::: warning NOTE
As of the current build, Kele Pool needs to have access to your wallet's private key in order to perform its automatic duties.
...
Your wallet's private key will be stored in a file located at `~/.kelepool/data/wallet`.

The password used to encrypt your wallet's key will be stored in `~/.kelepool/data/password`.
This is the file that must be protected at all times.
:::

::: tab Native Mode
The wallet and password files will be stored in the `data` directory under the Kele Pool directory that you set up earlier (e.g. `/srv/kelepool`).

Your wallet's private key will be stored in a file located at `/srv/kelepool/data/wallet`.

The password used to encrypt your wallet's key will be stored in `/srv/kelepool/data/password`.
This is the file that must be protected at all times.
:::
::::
...
To get an actual ETA of the sync time, it's easiest to look at the logs for your clients with `kelepool service logs eth1` and `kelepool service logs eth2` respectively (or the equivalent log script commands if you're using the hybrid / native modes.)
