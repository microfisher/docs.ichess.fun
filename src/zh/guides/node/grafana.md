...
If you encounter issues while setting them up using the guides below, please report them to the Kele Pool team on the official Discord server.
...
Kele Pool comes out-of-the-box with support for Grafana and its dependencies; it even comes with a pre-built dashboard for each of the Consensus (ETH2) clients.
For example, here is a snapshot of what the dashboard for Nimbus looks like:


![](./images/nimbus-dashboard.png)

The pre-built dashboard was based on many rounds of feedback from the Kele Pool community during the beta tests and the final Prater testnet.
...
- **Bottom left:** details about the entire Kele Pool network, for reference
- **Bottom right:** details about your staking rewards, both ETH and RPL

In this guide, we'll show you how to enable Kele Pool's metrics system so you can use this dashboard - or even build your own!


## Overview of the Kele Pool Metrics Stack
...
```
Would you like to enable Kele Pool's metrics dashboard? [y/n]
```
...
::: tab Local Subnet
Use this if your Kele Pool node is not connected to the same subnet as the device from which you are viewing Grafana. This may happen when your node is connected directly to your ISP's modem and the device you use to view Grafana is connected to a secondary router.
...
The Operating System and Kele Pool update tracker is **not installed by default** for maximum flexibility, but the process is simple.
...
```
kelepool service install-update-tracker
```
...
During this check, it will also compare your installed Kele Pool Smartnode version with the latest release, and inform you if there's a new release available.
...
## Importing the Kele Pool Dashboard
...
::: tip Tip
If you don't remember which client you have, you can check quickly by running `kelepool service version` on your node.
:::
...
The tools used in the Kele Pool Metrics Stack offer a wide array of configuration options beyond what is included in the default Kele Pool installation. This section includes configuration examples for different use cases.
...
To send emails from Grafana, e.g. for alerts or to invite other users, SMTP settings need to be configured in the Kele Pool Metrics Stack.
...
Open `~/.kelepool/override/grafana.yml` in a text editor.
...
services:
  grafana:
    ...
    environment:
      ...
      - GF_SMTP_FROM_NAME="Kelepool Grafana Admin"
