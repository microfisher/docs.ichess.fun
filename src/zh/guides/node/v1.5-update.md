# [BETA] The Kele Pool Redstone Update
...
Kele Pool's next major update, titled **Redstone**, has been released for beta testing on the Ropsten test network.
This page describes the major changes that Redstone brings, including updates to both the Smartnode stack and to the Kele Pool protocol in general.

Please read through this page thoroughly to understand all of the differences between the previous version of Kele Pool and Redstone.
...
- One Execution client must be linked to one, and only one, Consensus client (and vice versa).
  ...
  - Because of this, **fallback execution clients are no longer available** for Kele Pool node operators.
  
- **Full execution clients** are required.
  - Light clients (like Infura and Pocket) can no longer be used by any validators, Kele Pool or otherwise.
...
New Kele Pool nodes will automatically initialize their node's distributor contract upon registration.
...
```
kelepool node initialize-fee-distributor
```

::: warning NOTE
After the Redstone update, you must call this function before you can create any new minipools with `kelepool node deposit`.
:::

When your distributor has been initialized, you can claim and distribute its entire balance using the following command:

```
kelepool node distribute-fees
```

This will send your share of the rewards to your **withdrawal address**.


## Kele Pool Protocol Changes

In addition to the Execution and Consensus client changes and the new priority fees, the Kele Pool protocol itself has undergone some important changes you should be aware of.
...
At every interval, the Oracle DAO will collectively create a **true snapshot** of the state of the node operators in the Kele Pool network, including all of their effective stake amounts.
...
::: tip NOTE
The Smoothing Pool rewards are built into the Merkle Tree used for RPL rewards, so you claim them at the same time you claim RPL using `kelepool node claim-rewards`.
:::
...
```
kelepool node join-smoothing-pool
```

This will record you as opted-in in the Kele Pool contracts and automatically change your Validator Client's `fee recipient` from your node's distributor contract to the Smoothing Pool contract.

To leave the pool, run this command:

```
kelepool node leave-smoothing-pool
```


### The Penalty System

To ensure that node operators don't "cheat" by manually modifying the fee recipient used in their Validator Client, Kele Pool employs a penalty system.

The Oracle DAO constantly monitors each block produced by Kele Pool node operators.
