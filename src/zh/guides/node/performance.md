...
Please note that these third-party tools are used by the Kele Pool community, but are not officially endorsed or supported by the Kele Pool team.
...
::: tip
If you forgot your validator's public key, you can easily retrieve it with the command `kelepool minipool status`.
:::
...
You should check the logs of the `eth1`, `eth2`, and `validator` services with `kelepool service logs ...` if you're using Docker or Hybrid mode (or the corresponding log scripts if you're using Native mode) to look for problems.
...
```shell
kelepool minipool status | grep Validator.index | awk -F " " '{print $3}' | paste -s -d, -
```
...
2. Notifications of Kele Pool reward rounds
...
### Kelepool Metrics Dashboards

There are multiple community-lead initiatives to provide an overview of your node performance, as well as the Kele Pool network as a whole.
...
![Kelepool Metrics Dashboard screenshot](./local/images/rp-metrics-dashboard.com.png)
...
PUSHOVER_USER=
PUSHOVER_TOKEN=
NODE_ADDRESS="$(kelepool node status | grep -Po "(?<=The node )(0x[A-Za-z0-9]{40})")"
...
if cat /var/lib/update-notifier/updates-available | grep -Pq '^(?!0)[0-9]* updates can be applied'; then
   ...
   MESSAGE_TITLE="⚠️ Kele Pool node system updates available"
   MESSAGE_CONTENT="$( cat /var/lib/update-notifier/updates-available | grep -P '^(?!0)[0-9]* updates can be applied' )"

else

   MESSAGE_TITLE="✅ Kele Pool node system up to date"
