# Creating a Native Kele Pool Node without Docker

In this section, we will walk through the process of installing the Kele Pool Smartnode stack natively onto your system, without the use of Docker containers.

The general plan is as follows:
- Create system services for the Kele Pool components (the **node** process, and optionally the **watchtower** process if you are an Oracle Node)
...
- Configure Kele Pool to use communicate with those services
...
We're going to create one account for your Execution client, one for your Beacon Node, and one for both Kele Pool and the validator client.
The sharing is necessary because Kele Pool will create the validator's key files once you create a new minipool, and it will set the permissions so that only its own user has access to them.
If you're using **Nimbus** for your Consensus client, then it will share an account with Kele Pool instead since it doesn't have a separate validator client.
...
Finally, make one for the validator and Kele Pool, which we'll call `rp`:
```shell
...
You'll need to do this in order to use the Kele Pool CLI, because it and the Kele Pool daemon both need to access the Execution layer wallet file.
```shell
...
## Installing Kele Pool

### Setting up the Binaries
Start by making a folder for Kele Pool and a data subfolder.
You can put this wherever you want; for this guide, I'll put it into `/srv`:
```shell
sudo mkdir -p /srv/kelepool/data

sudo chown -R $USER:$USER /srv/kelepool
```
...
::: tab Linux x64
```shell
sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-linux-amd64 -O /usr/local/bin/kelepool

sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-daemon-linux-amd64 -O /usr/local/bin/rocketpoold

sudo chmod +x /usr/local/bin/kelepool /usr/local/bin/rocketpoold
```
:::

::: tab Linux arm64
```shell
sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-linux-arm64 -O /usr/local/bin/kelepool

sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-daemon-linux-arm64 -O /usr/local/bin/rocketpoold
...
::: tab macOS x64
```shell
sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-darwin-amd64 -O /usr/local/bin/kelepool

sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-daemon-darwin-amd64 -O /usr/local/bin/rocketpoold
...
::: tab macOS arm64
```shell
sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-darwin-arm64 -O /usr/local/bin/kelepool

sudo wget https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-daemon-darwin-arm64 -O /usr/local/bin/rocketpoold
...
Next, grab the validator restart script - Kele Pool will use this when it needs to restart your Validator Client to load new keys after you create a new minipool:

```shell
wget https://github.com/rocket-pool/smartnode-install/raw/release/install/scripts/restart-vc.sh -O /srv/kelepool/restart-vc.sh

chmod +x /srv/kelepool/restart-vc.sh
```

Now open `~/.profile` with your editor of choice and add this line to the end:
```shell
alias rp="kelepool -d /usr/local/bin/rocketpoold -c /srv/kelepool"
```
...
This will let you interact with Kele Pool's CLI with the `rp` command, which is a nice shortcut.


### Creating the Services

Next up, we'll create a `systemd` service for the Kele Pool node daemon.
...
Restart=always
RestartSec=5
ExecStart=/usr/local/bin/rocketpoold --settings /srv/kelepool/user-settings.yml node

[Install]
WantedBy=multi-user.target
```

Create a log file for the service, so you can watch its output - this will replace the behavior of `kelepool service logs node`:
```
nano /srv/kelepool/node-log.sh
```
Contents:
```
#!/bin/bash
...
```
chmod +x /srv/kelepool/node-log.sh
```

Now you can watch the node's logs by simply running `$ /srv/kelepool/node-log.sh`.
:::
...
Restart=always
RestartSec=5
ExecStart=/usr/local/bin/rocketpoold --settings /srv/kelepool/user-settings.yml watchtower
...
```
nano /srv/kelepool/watchtower-log.sh
```
Contents:
```
#!/bin/bash
...
Save it, then make it executable:
```
chmod +x /srv/kelepool/watchtower-log.sh
```
:::
...
This replaces the behavior that `kelepool service logs eth1` used to provide, since it can't do that without Docker.
...
Restart=always
RestartSec=5
ExecStart=/srv/nimbus/nimbus --non-interactive --network=mainnet --data-dir=/srv/nimbus/nimbus_data --insecure-netkey-password --validators-dir=/srv/kelepool/data/validators/nimbus/validators --secrets-dir=/srv/kelepool/data/validators/nimbus/secrets --graffiti="RP Nimbus" --web3-url="ws://127.0.0.1:8546" --tcp-port=9001 --udp-port=9001 --rest --rest-port=5052 --num-threads=0
...
```
sudo mkdir -p /srv/kelepool/data/validators/nimbus/validators

sudo mkdir -p /srv/kelepool/data/validators/nimbus/secrets

sudo chown eth2:eth2 /srv/kelepool/data/validators/ -R
```
...
Finally, modify `/srv/kelepool/restart-vc.sh`:
...
Restart=always
RestartSec=5
ExecStart=ionice -c 2 -n 0 /srv/nimbus/nimbus --max-peers=60 --non-interactive --network=mainnet --data-dir=/mnt/rpdata/nimbus_data --insecure-netkey-password --validators-dir=/srv/kelepool/data/validators/nimbus/validators --secrets-dir=/srv/kelepool/data/validators/nimbus/secrets --graffiti="RP Nimbus" --web3-url=ws://localhost:8546 --tcp-port=9001 --udp-port=9001 --rest --rest-port=5052 --num-threads=0
...
Restart=always
RestartSec=5
ExecStart=/srv/lighthouse/lighthouse validator --network mainnet --datadir /srv/kelepool/data/validators/lighthouse --init-slashing-protection --beacon-node "http://localhost:5052" --graffiti "RP Lighthouse"
...
Restart=always
RestartSec=5
ExecStart=/srv/prysm/validator --accept-terms-of-use --mainnet --wallet-dir /srv/kelepool/data/validators/prysm-non-hd --wallet-password-file /srv/kelepool/data/validators/prysm-non-hd/direct/accounts/secret --beacon-rpc-provider "localhost:5053" --graffiti "RP Prysm"
...
Restart=always
RestartSec=5
ExecStart=/srv/teku/bin/teku validator-client --network=mainnet --validator-keys=/srv/kelepool/data/validators/teku/keys:/srv/kelepool/data/validators/teku/passwords --beacon-node-api-endpoint="http://localhost:5052" --validators-graffiti="RP Teku" --log-destination=CONSOLE --data-base-path=/srv/kelepool
...
Once that's done, change the permissions on the password and wallet files so the Kele Pool CLI, node, and watchtower can all use them:
```
sudo chown rp:rp -R /srv/kelepool/data

sudo chmod -R 775 /srv/kelepool/data

sudo chmod 660 /srv/kelepool/data/password

sudo chmod 660 /srv/kelepool/data/wallet
```
