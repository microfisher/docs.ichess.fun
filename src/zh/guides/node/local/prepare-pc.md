...
Before installing Kele Pool, there are a few checks you should do to make sure your system is compatible and will work correctly.

::: danger
We strongly encourage you to create a dedicated machine for running a Kele Pool node.
...
Below is a brief description of the software and hardware requirements that a Kele Pool node requires.
This guide assumes that you already have your machine physically built, and the operating system installed.


### Supported Operating Systems

Kele Pool's Smartnode client currently supports **Linux** and **macOS** systems.

At this time, **Windows** can be used to remotely manage a remote Linux or Mac machine, but the Smartnode itself cannot currently run on a Windows system. However, Kele Pool *can* be run on a Linux [virtual machine](https://en.wikipedia.org/wiki/System_virtual_machine) hosted by a Windows machine.
...
Kele Pool is natively compatible with **AMD64 (x64)** and **arm64 (aarch64)** CPU architectures.
...
There are many variants of the Linux OS (called distributions, or **distros** for short). While you can run Kele Pool from any modern distro, Kele Pool's installer can automatically install the entire stack on [Ubuntu](https://ubuntu.com/about), [Debian](https://www.debian.org/intro/why_debian), [CentOS](https://www.centos.org/about/), and [Fedora](https://docs.fedoraproject.org/en-US/project/).
...
Before installing Kele Pool, please review the following checklist:
...
If you have checked and confirmed all of these items, then you are ready to install Kele Pool and begin running a node!
