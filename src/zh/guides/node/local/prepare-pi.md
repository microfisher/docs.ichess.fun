...
This guide will walk you through how run a Kele Pool node using a Raspberry Pi.
...
This setup will run **a full Execution (ETH1) node** and **a full Consensus (ETH2) node** on the Pi, making your system contribute to the health of the Etherum network while simultaneously acting as a Kele Pool node operator.
...
To run a Kele Pool node on a Raspberry Pi, you'll need to first have a working Raspberry Pi.
...
These are the recommended components that you'll need to buy in order to run Kele Pool on a Pi:
...
- A **case**. There are two ways to go here: with a fan, and fanless.
  - With a fan:
    - A 40mm **fan**. Same as the above, the goal is to keep things cool while running your Kele Pool node.
...
Once that's complete, you're ready to start preparing Ubuntu to run a Kele Pool node.
...
If your SSD is too slow, then it won't work well for a Kele Pool node and you're going to end up losing money over time.
...
For example, this has been experienced by the Kele Pool community with the Samsung T7.
...
And with that, your Pi is up and running and ready to run Kele Pool!
