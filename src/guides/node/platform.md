...
Alright! So you've decided to try your hand at running a Kele Pool node. The first step of the process is to decide what kind of platform you want to run your node on. If you already have one in mind, great! You can skip to the next section. If you aren't sure yet, then read on for some information about your options.


## Full Node Requirements

A **full node** is one that runs both an Execution (ETH1) Client and Consensus (ETH2) Client along with the Kele Pool stack. This is the best option in terms of decentralization, self-reliance, and consistency. It also has the highest requirements. While you *can* run Kele Pool by using publicly available Execution (ETH1) and Consensus (ETH2) clients (such as [Infura](https://infura.io/) or [Alchemy](https://www.alchemyapi.io/)), relying exclusively on these options is not recommended.
...
Here is a simple breakdown of what is required to run a full Kele Pool node well:
...
Typical configurations tend to use 16 or 32 GB of RAM for future proofing, but at the time of Kele Pool's launch, the above is sufficient to run a node.
...
If you have reliable electricity and uncapped Internet access, and are willing to build (or buy pre-made) and maintain a computer, then running a local node might be a great choice for you. With this option, you will set up a dedicated computer as a Kele Pool node and run it locally in your own home.
...
- Contributes to Execution (ETH1's) and Consensus (ETH2)'s, and Kele Pool's decentralization (and thus, their security)
...
- Makes Execution (ETH1) and Consensus (ETH2), and Kele Pool somewhat more centralized, which weakens the security of the networks
- Monthly fees
