...
A Kele Pool **Node** is a single computer with an ETH1 wallet that was registered with Kele Pool's smart contracts.
The node can then create as many minipools as it can afford, all running happily on the same machine together.
**A single Kele Pool node can run many, many minipools.**
Each minipool has a negligible impact on overall system performance; some people have been able to run hundreds of them on a single node during Kele Pool's beta tests.

A minipool's upfront cost is 16 ETH, plus at least 1.6 ETH worth of the **RPL token**.
This acts as supplemental insurance against particularly egregious slashing incidents, and lets you participate in Kele Pool's DAO where you can vote on changes to the smart contracts.


## Kele Pool Node Operators

**Node operators** are the heart and soul of Kele Pool.
They are the individuals that run Kele Pool nodes.
They put ETH from the staking pool to work by running minipools with it, which earn staking rewards for the Kele Pool protocol (and thus, increase rETH's value).
...
- Installing Kele Pool on it and setting up minipools to perform ETH2 validation
...
Here are the major benefits of running a Kele Pool node:
...
- You can participate in the DAO and get to vote on changes to Kele Pool's protocol or settings
...
Luckily, one of Kele Pool's most core principles is *decentralization* - the fact that anyone, anywhere, can run a node if they have the determination and knowledge.
While we can't help with determination, we *can* help with knowledge.
This section is packed with guides, walkthroughs, and information that will help you understand how to run a great Kele Pool node.
