...
They are designed to significantly improve your node operator experience and address many of the features that our users have requested since Kele Pool's launch.
...
- The `docker-compose` YAML files have been removed. Each container now gets its own file in the `templates` folder. Upon `kelepool service start`, these templates are filled out with your settings and put into the `runtime` folder automatically.
...
```shell
curl -L https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-cli-darwin-arm64 > /usr/local/bin/kelepool
```
:::
:::: tabs
...
curl -L https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-daemon-darwin-amd64 > /usr/local/bin/rocketpoold
```
:::
::: tab macOS arm64
For `arm64` systems, such as the Mac mini with M1:
...
curl -L https://github.com/rocket-pool/smartnode-install/releases/latest/download/kelepool-daemon-darwin-arm64 > /usr/local/bin/rocketpoold
```
:::
:::: tabs
...
For example, if you followed the Native setup guide and your Smartnode directory is `/srv/kelepool`:
...
```
ExecStart=/home/ubuntu20/bin/rocketpoold --settings /srv/kelepool/user-settings.yml node
```
...
After the installation, all of your old configuration files will be moved to a folder named `old_config_backup` in your Smartnode directory (by default `~/.kelepool/old_config_backup`).
...
In the next step, you should run `kelepool service config`.
...
Press `Enter` to be brought to the **Review Page**, where you can see everything you've changed since the last time you ran `kelepool service config`.
...
Use the `kelepool service version` command to check that you have the correct version of the Smartnode and client containers:
...
Kele Pool client version: 1.3.0-rc1
Kele Pool service version: 1.3.0-rc1
...
Use the `kelepool service logs eth1` and `kelepool service logs eth2` commands to watch the logs for your Execution client (formerly ETH1 client) and your Consensus client (formerly ETH2 client).
Look out for any errors and [please visit our Discord server for help](https://discord.com/invite/kelepool) if you encounter them.
...
```shell
wget https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-cli-linux-amd64 -O ~/bin/kelepool
```
:::
::: tab Linux arm64
For `arm64` systems (like the Raspberry Pi):
...
```shell
wget https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-cli-linux-arm64 -O ~/bin/kelepool
```
:::
::: tab macOS x64
For `x64` systems (most Macs):
```shell
curl -L https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-cli-darwin-amd64 > /usr/local/bin/kelepool
```
:::
::: tab macOS arm64
For `arm64` systems, such as the Mac mini with M1:
```shell
curl -L https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-cli-darwin-arm64 > /usr/local/bin/kelepool
```
:::
:::: tabs
...
```shell
sudo wget https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-cli-linux-amd64 -O /usr/local/bin/kelepool

sudo wget https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-daemon-linux-amd64 -O /usr/local/bin/rocketpoold
```
:::
::: tab Linux arm64
For `arm64` systems (like the Raspberry Pi):
...
```shell
sudo wget https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-cli-linux-arm64 -O /usr/local/bin/kelepool
...
curl -L https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-daemon-darwin-amd64 > /usr/local/bin/rocketpoold
```
:::
::: tab macOS arm64
For `arm64` systems, such as the Mac mini with M1:
...
curl -L https://github.com/rocket-pool/smartnode-install/releases/download/v1.2.4/kelepool-daemon-darwin-arm64 > /usr/local/bin/rocketpoold
```
:::
:::: tabs
...
```
ExecStart=/usr/local/bin/rocketpoold --config /srv/kelepool/config.yml --settings /srv/kelepool/settings.yml node
```
