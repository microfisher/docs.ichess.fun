...
If you've already used Kele Pool on our Prater test network, are comfortable with its ins and out, and are ready to either stake ETH or run a node for real on the main Ethereum chain, then this guide is for you!
...
- If you are using Geth, your node's workload will be **considerabily higher**. Geth takes approximately 20x the storage space of Goerli (400 GB as of 2021-09-05), and requires more CPU power and RAM to process. If you're using the Kele Pool Grafana dashboard, be prepared to see much higher usage.
...
- The RPL price used by the Kele Pool network (and thus, your collateral level) along with the total effective staked RPL across the network are reported **once every 24 hours** instead of once every hour.
...
Would you like the Smartnode to automatically switch networks for you? This will destroy and rebuild your `data` folder and all of Kele Pool's Docker containers. [y/n]
```
...
4. Delete your Kele Pool configuration folder:
   ```
   sudo rm -rf ~/.kelepool
   ```

5. (Optional) Delete your Kele Pool CLI:
   ```
   rm ~/bin/kelepool
   ```
...
2. Stop all of the Kele Pool and Ethereum services (using, for example, `geth` and `lighthouse`; replace with the services you created during installation):
   ```
...
4. Delete the Kele Pool configuration folder:
   ```
   sudo rm -rf /srv/kelepool
   ```

5. (Optional) Remove the Kele Pool CLI and Daemon binaries:
   ```
   sudo rm /usr/local/bin/kelepool /usr/local/bin/rocketpoold
   ```
...
- Create the `/srv/kelepool` folder and its skeleton framework, and set the permissions properly
...
- Get the latest configuration files from the installer archive, put them into `/srv/kelepool`, and adjust them to match your node's setup
:::
