...
Specifically, we'll cover how to select the appropriate virtual hardware configuration and how to prepare the Operating System for Kele Pool.  
...
For a Kele Pool node, we recommend you use the **Ubuntu Server 20.04 LTS (HVM)** image.
...
- Use at least **100 GiB** if you're just trying Kele Pool out on the [Prater Test Network](../../testnet/overview.md).
...
In the **Add Tags** step, you should click the **click to add a Name tag** button and give it an easy-to-remember name that you can use to identify the server later; something like **Kele Pool Smartnode** will work if you need inspiration.
...
If anyone gets a hold of this key, they will have complete access to your node - **including your Kele Pool node wallet** and all of the coins within it.
...
You'll see your Kele Pool node there with some accompanying details about its configuration and status.
...
Leave the resource type set to **Instance** and select the Kele Pool Smartnode instance you just created from the list.
...
And with that, your server is up and running and ready to run Kele Pool!
