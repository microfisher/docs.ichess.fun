...
As a node operator, the CLI is your primary tool for interacting with Kele Pool.
...
```
NAME:
   kelepool - Kele Pool CLI

USAGE:
   kelepool [global options] command [command options] [arguments...]

VERSION:
   1.1.1

AUTHORS:
   David Rugendyke <david@kelepool.net>
   Jake Pospischil <jake@kelepool.net>
   Joe Clapis <joe@kelepool.net>
   Kane Wallmann <kane@kelepool.net>

COMMANDS:
   auction, a   Manage Kele Pool RPL auctions
   faucet, f    Access the legacy RPL faucet
   minipool, m  Manage the node's minipools
   network, e   Manage Kele Pool network parameters
   node, n      Manage the node
   odao, o      Manage the Kele Pool oracle DAO
   queue, q     Manage the Kele Pool deposit queue
   service, s   Manage Kele Pool service
   wallet, w    Manage the node wallet
   help, h      Shows a list of commands or help for one command

GLOBAL OPTIONS:
   --allow-root, -r              Allow kelepool to be run as the root user
   --config-path path, -c path   Kele Pool config asset path (default: "~/.kelepool")
   --daemon-path path, -d path   Interact with a Kele Pool service daemon at a path on the host OS, running outside of docker
...
The `service` group is all about managing the Kele Pool services themselves.
You've already done some of this during the initial setup process.
To see them all, run:

```
kelepool service help
```
...
```
NAME:
   kelepool service - Manage Kele Pool service

USAGE:
   kelepool service [global options] command [command options] [arguments...]

VERSION:
   1.1.1

COMMANDS:
   install, i                 Install the Kele Pool service
   config, c                  Configure the Kele Pool service
   status, u                  View the Kele Pool service status
   start, s                   Start the Kele Pool service
   pause, p                   Pause the Kele Pool service
   stop, o                    Pause the Kele Pool service (alias of 'kelepool service pause')
   terminate, t               Stop the Kele Pool service and tear down the service stack
   logs, l                    View the Kele Pool service logs
   stats, a                   View the Kele Pool service stats
   version, v                 View the Kele Pool service version information
...
GLOBAL OPTIONS:
   --compose-file value, -f value  Optional compose files to override the standard Kele Pool docker-compose.yml; this flag may be defined multiple times
   --help, -h                      show help
```
...
This command shows you the current running status of each of the Docker containers managed by Kele Pool.
...
rocketpool_node         /go/bin/kelepool node         Up                                                                                                                    
rocketpool_validator    sh /setup/start-validator.sh    Up                                                                                                                    
rocketpool_watchtower   /go/bin/kelepool watchtower   Up
```
...
They simply start all of the Kele Pool containers, or stop them.

::: tip
The `pause` command does the same thing as `stop`.
It's just left over as a legacy command from earlier versions of Kele Pool.
:::
...
If you simply do `kelepool service logs` without any other arguments, it will aggregate all of the logs together and show them to you at once.
...
You can call this command at any time, but the changes won't take effect until you call `kelepool service stop` and `kelepool service start`.


### `terminate`

This command will shut down the Docker containers, then delete them, delete the Kele Pool virtual network, and delete the ETH1 and ETH2 chain data volumes.
It essentially removes all of the Kele Pool items from your Docker setup.
Use it when you want to clean up that portion of the Kele Pool installation.
...
To remove those, you will need to delete the `~/.kelepool/data` folder in Docker or Hybrid Mode, or the corresponding directory in Native Mode.
:::


## Node Commands

The `node` group involves operations on your Kele Pool node.
...
Here is what the `kelepool node help` output will show:

```
NAME:
   kelepool node - Manage the node

USAGE:
   kelepool node [global options] command [command options] [arguments...]
...
COMMANDS:
   ...
   register, r                    Register the node with Kele Pool
...
This is an example of what `kelepool node status` shows once you have your node registered and some minipools set up:

```
The node <node address> has a balance of 852.221778 ETH and 4820.395655 RPL.

The node is registered with Kele Pool with a timezone location of <timezone>.
...
The output of `kelepool node sync` will look like this:
...
This command lets you send ETH, RPL, or other Kele Pool-related tokens from the node wallet to a different address.
...
```
kelepool node send <amount> <token> <address>
```
...
```
kelepool node send 1 eth <my friend's address>
```
...
Here is what the `kelepool minipool help` output will show:

```
NAME:
   kelepool minipool - Manage the node's minipools

USAGE:
   kelepool minipool [global options] command [command options] [arguments...]
...
This command lets you pull 16 ETH back from a minipool if you deposited 32 ETH to create one, once Kele Pool was able to contribute 16 ETH from the rETH staking pool.
...
By default, Kele Pool will use an oracle to look at the current transaction pool and suggest a reasonable max fee for any transactions you trigger.
...
To do this, add them after `kelepool` and before the other command information.

For example, calling `node set-timezone` with this flag would provide the following output:

```
$ kelepool -f 10 node set-timezone
...
Since all of your subsequent transactions will wait until that one goes through, that transaction essentially blocks all of the operations on your Kele Pool node.
...
Once you have it, simply call any transaction with the CLI using the `--nonce <value> -i 2.2` flags after `kelepool` and before the rest of the command.
...
```
$ kelepool --nonce 10 -f 150 -i 2.2 node send 0.0001 eth <node wallet address>
