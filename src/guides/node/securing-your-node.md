...
# This should be fine since Kelepool auto-starts on reboot.
...
::: tip NOTE
If you selected a different execution/consensus client port during the Kelepool setup, you need to edit the ports below to reflect your settings.
:::
...
```shell
sudo ufw allow 30303/tcp comment 'Execution client port, standardized by Kele Pool'
sudo ufw allow 30303/udp comment 'Execution client port, standardized by Kele Pool'
```

Allow consensus client (formerly referred to as ETH2):
```shell
sudo ufw allow 9001/tcp comment 'Consensus client port, standardized by Kele Pool'
sudo ufw allow 9001/udp comment 'Consensus client port, standardized by Kele Pool'
```
