...
This guide will introduce you to how staking with Kele Pool works (how your ETH is put to work and your rewards are generated) and summarize the ways in which you can stake.
...
Before getting into Kele Pool, staking on the [Beacon Chain](https://ethereum.org/en/eth2/beacon-chain/) (ETH2) is done via **validators**.
...
Unlike solo stakers, who are required to put 32 ETH up for deposit to create a new validator, Kele Pool nodes only need to deposit 16 ETH per validator.
...
All of the creation, withdrawing, and rewards delegation is handled by Kele Pool's **smart contracts** on the ETH1 chain.
This makes it completely decentralized.


## The rETH Token

As a Kele Pool staker, your role is to deposit ETH into the deposit pool which will enable a node operator to create a new Beacon Chain validator.
...
The rETH/ETH exchange rate is updated approximately every 24 hours based on the Beacon Chain rewards earned by Kele Pool node operators.
...
Say 128 ETH had been staked with Kele Pool and the sum of all validator balances on ETH2 was 160 ETH.
Then 1 ETH would be worth (128/160) = 0.8 rETH; conversely, 1 rETH would be worth (160/128) = 1.25 ETH.

At this point, you could trade your 10 rETH back to Kele Pool's smart contracts and receive 12.5 ETH in return.

This means **as long as you are holding rETH**, you are staking with Kele Pool!
**You do not need to get it from Kele Pool directly.**
...
::: warning NOTE
Trading rETH back for ETH directly with Kele Pool is only possible when the staking pool has enough ETH in it to handle your trade.
...
As an alternative to holding onto and eventually returning your rETH to the Kele Pool, you are also free to **use it in DeFi applications**.
...
## How to Stake with Kele Pool

As described above, as long as you are holding the rETH token, you are a **Kele Pool Staker**.
...
If you have never used an Ethereum wallet before, we strongly encourage you to learn more about them and practice using them before staking with Kele Pool.
...
### Via Kele Pool Directly
With this method, you will use a web3 wallet (such as MetaMask) and interact with Kele Pool's website to swap ETH for rETH and vice-versa.
This method is **guaranteed** to provide you with the exact amount of rETH that your ETH is worth, since it's coming directly from Kele Pool's smart contracts, but it can be somewhat expensive if the network's gas prices are high and you're staking a relatively small amount of ETH.
...
[Click here to learn how to stake via Kele Pool directly.](./via-rp.md)
...
Kele Pool has officially created a [Balancer metastable pool](https://docs.balancer.fi/products/balancer-pools/metastable-pools).
...
Because of this, and because it has a **smaller transaction fee** than swapping directly via Kele Pool's website, we **strongly recommended** using Balancer if you want to stake via this route.

If Balancer doesn't work for you, there is also an ETH-rETH liquidity pool on Uniswap.
Note that this is a conventional pool, so **you will be affected by slippage and higher fees** if you use it, but the gas fee will still be lower than staking directly via Kele Pool's website.
